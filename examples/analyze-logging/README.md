...
**Note:** This example was generated by agentic-analyzer v0.1.0 and is superseded by v0.2.0. Regenerate it by running `/new-analyzer` inside a Claude Code session — the old `logging-config.json` file no longer exists and the `config.json`-driven workflow has been replaced by an interactive interview.
